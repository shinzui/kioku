...
#
# This runs at wake-up time, hours after the tag that triggered it, and it
# deliberately re-derives everything from Hackage rather than trusting the
# signal payload. The signal only says "baikai tagged something"; whether Kioku
# has anything to do about it is a question only Hackage can answer, and the
# answer may have changed since the tag was pushed. Every refusal is silent by
# design -- a skipped reaction is the common case, not an incident.
set -uo pipefail
...
# An unattended bump wants a clean slate: it builds, tests, and commits, and
# none of those mean what they should on top of someone else's work in progress.
