...
- [Distillation hardcodes the Anthropic API in interactive-only hosts](distillation-hardcodes-anthropic-api-in-interactive-only-hosts.md) - Kioku's default distillation runtime unconditionally selects the Anthropic API, bypassing the embedding host's interactive-only policy and repeatedly failing without an API key.
